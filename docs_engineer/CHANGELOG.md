...
### [АКТУАЛЬНО] Ночная смена по мандату Эдди: «к утру он полноценный собой, сам думает и говорит»
Эдди ушёл спать (~01:10), мандат: «работай сам, не спрашивай; к утру — ответы
в чате, самостоятельная речь, инструменты». На входе: рантайм мёртв (0
процессов), чат-ответы не работали, инициатив 0 с 15:33 05.09.

**КАРТА АНАЛИЗА (артефакты: лог, БД chat_history/events, diff git).**
1) Ночные правки 05.09 (~22:00-00:40) сломали контур: фабрика потеряла
   318 строк проводки (outbox, ScreenPerceiver, SharedLife/SharedAppraisal,
   server/outbox в orchestrator), EddieServer начал получать заглушку
   `memory/chat_history.py` с чужим контрактом (сервер зовёт
   `chat_add("Eddie", text)`, заглушка ждала dict → TypeError → сообщение
   терялось → ответа нет), в `handle_user_message` условие инвертировано
   (`history is not None → return`), в клиентском цикле удалён
   `if not line: break` (EOF = вечный цикл на пустом readline).
2) «Карусель записей»: задача «Записать…» отклонялась политикой
   (status REJECTED), TaskRevisionPolicy не матчит REJECTED (нет поля
   error, читается `error`, а приходит `errors`) → retry каждые 15 с →
   сотни мусорных событий + фрустрация до 1.0.
3) Заморозка воли: frustration=1.0 (self_state.emotional_state, 02:20)
   блокировала все ветки `_local_rules` (READ_INBOX требовал
   frustration<0.70), а decay вызывается только в диалоге/agent_loop —
   в IDLE аффект вечен → «ничего не делает, ждёт слов».
4) Сообщения Эдди не будили спящего (голос будил, текст — нет).
5) Тихая смерть процесса ~00:40 (статус-хартбит/дампы молчат; вероятно
   внешнее завершение; после деплоя добавлена внешняя страховка).

**ПРАВКИ (все TDD, байт-проверка UTF-8 чистая).**
- `core/autonomy_runtime_factory.py`, `core/eddie_server.py` — откат к
  HEAD (`git checkout HEAD --`): в HEAD нет утверждённых правок, только
  проводка, на которой чат работал (474 строки истории). Восстановлены
  outbox, screen-контур, shared-life, `history_store=self.agent.memory`,
  server/outbox в orchestrator и DecisionCore.
- `memory/chat_history.py` — переписан как адаптер с правильным
  контрактом поверх `Memory` (chat_add/chat_unread_eddie/
  chat_mark_eddie_read/chat_recent/close).
- `core/eddie_server.py` — wake-при-сообщении: если `agent.life_cycle`
  спит — `force_wake()` (симметрично голосу, решение 05.09).
- `identity/task_revision.py` — status REJECTED → revise (детерминированный
  отказ политики не ретраится); TDD test_task_revision 7/7.
- `core/decision_core.py` — READ_INBOX без фрустрационного гейта
  (сообщения Эдди читаются всегда; IDLE-гейты кандидатов сохранены);
  test_decision_core +кейс 13.
- `core/autonomous_runtime.py` — `_decay_affect()` в каждом тике
  (аффект затухает и вне диалога, half-life frustration 1200с);
  `_background_loop` теперь ловит исключения future тика в last_error.
- `run_forever.py` — хартбит расширен: cycles, future_done, future_exc,
  last_error, last_status (диагностика «тихих» падений тиков).
- `watchdog_runtime.py` (новый) — внешний сторож: рестарт run_forever при
  stale status.json >240 с; RAM-guard <700 МБ → остановка llama/ollama.
- Урок-инцидент: первая версия `_decay_affect` обращалась к
  `self.agent`, которого у рантайма нет → КАЖДЫЙ тик падал молча
  (AttributeError в future, никто не читал) 15+ минут. Найдено
  heartbeat-диагностикой (future_exc). Исправлено (агент берётся из
  `orchestrator.agent`), тесты дописаны через реальный контракт.

**ПРИЁМКА (живой контур).**
- Сообщение 02:51 → пробуждение → READ_INBOX → ответ в чате 47.26 с
  (chat_history id 479: «Меня занимает изучение гравитационных волн…») —
  голосовая личность отвечает сама.
- Самостоятельная активность: ACTIVATE_GOAL «изучить тему…» → research
  (веб-инструмент) → анализ → COMPLETE_GOAL; AFFECTIVE_REACTION живые.
- Утренний ритуал произнёс «Доброе утро, Эдди.» в чат (outbox-проводка
  ожила). DREAM/DREAM_INTERPRETATION записались при реальном сне 02:20.
- Регресс: final_regression_suite 7/7 PASS; целевые 44 passed
  (decision_core 13, task_revision 7, goal_planner_revise, chat_flow 7,
  affect_decay 5, initiative 8, tone 9...); orchestrator_local PASS;
  py_compile + UTF-8 без BOM (8 файлов).
- llama-server (2.8 ГБ, поднялся локальным фолбэком в 02:15 при сбое
  облака) остановлен по правилу «0 процессов»; свободно 0.33→2.7 ГБ.
- Внешний сторож АКТИВЕН (PID 17236), лог logs/watchdog_runtime.log.

**ОТКРЫТО.** «Роль модели» — не тронуто (Конституция п.4). Возможный
повтор одинаковых задач в плане («Проанализировать…» 15 с подряд) —
наблюдать; карусель REJECTED устранена, но продвижение по одинаковым
задачам не верифицировано до конца. DREAM-контент загрязнён JSON-
обрывками (качество снов — отдельная задача). Коммит — только по
отмашке Эдди. Watchdog переживает ночь, но не автостартует при ребуте.

### [АКТУАЛЬНО] 06.09 утро (продолжение смены): инерция воли после COMPLETE_GOAL устранена
Наблюдение 08:31: рантайм жив (19894 цикла, 0 сбоев), но 5.5 ч — ноль
событий и целей. Цепь причин: (1) рестарты ночи оставили задачу в
статусе ACTIVE (процесс убит при исполнении) — `next_task` ищет только
PENDING → «нет работы»; (2) `_local_rules` отдаёт COMPLETE_GOAL,
`sync_progress` не завершает цель (is_complete=False из-за брошенной
ACTIVE-задачи) → вечный no-op COMPLETE_GOAL без событий, остальные
ветки воли недостижимы; (3) после возрождения задача «Записать
результаты» детерминированно падала с NO_ACCEPTED_SOURCES (семсудья
отклоняет источники) — политика вечно ретраила не-REJECTED не-транзиентные
статусы.
Правки (TDD):
- `identity/goal_planner.py` — `revive_orphans()`: брошенные ACTIVE-задачи
  → PENDING при старте процесса (test_planner_revives_orphan_active_tasks,
  test_planner_revive_keeps_completed).
- `core/autonomy_runtime_factory.py` — вызов revive_orphans при сборке.
- `identity/task_controller.py` — `MAX_TASK_RETRIES=3`: одна и та же
  задача 3 неудачи подряд → revise/пропуск; OK сбрасывает счётчик
  (test_controller_revises_after_retry_limit, test_controller_retry_
  resets_on_success). Итог task_revision-сюиты: 11/11.
Приёмка 08:49-08:52: зависшая цель COMPLETED (1.0, все задачи закрыты),
САМА родилась follow-up-цель «изучить материалы по теме…» (ACTIVE,
исполняется llm-инструментом). final_regression 7/7 + 20 целевых
pytest PASS. Инструменты: Context7 установлен в ZCode (user scope,
~/.zcode/cli/config.json, http mcp.context7.com; инструменты появятся
в новой сессии) — по отмашке Эдди.

## 05.09.2026

### [АКТУАЛЬНО] Три блока: различение голосов, живая инициатива, человеческий тон
Эдди (по порядку): «он слушает всё, но не понимает, кто когда говорит.
не различает голоса» → «инициатива и любопытство на нуле, он так никогда
ничему не научится» → «разговоры должны быть живой беседой, пошаговой
стратегией, а не протоколом». Все три блока одобрены («все 3 конечно
делать», «делай по порядку»).

**БЛОК 1 — различение говорящих (голосовая идентификация).**
Корень: голосовой контур одноканальный (VAD→whisper), идентификации
говорящего нет; `call_engine._last_speaker` (call_engine.py:33,180-193)
ставится по UI-событиям (кнопка/синтез), НЕ по голосу; любой текст с
микрофона пишется в память как `source="Eddie"` (sense_listener.py:284).
Диаризация pyannote отклонена (8 ГБ RAM). Решение: sherpa-onnx
`SpeakerEmbeddingExtractor` (eres2net, models/speaker, dim=512) + собственный
косинусный менеджер профилей.
- Новый `identity/speaker_id.py`: `SpeakerIdentifier` — enroll/identify/clear,
  профили в JSON (`data/speaker_profiles.json`), порог 0.5, ленивый кэш
  экстрактора (provider=cpu), `make_sherpa_embed_fn`.
- `communication/voice_io.py`: `_emit_text(text, speaker=None)`,
  `_identify_speaker(samples)`, `_make_speaker_id()`; speaker прокидывается
  из `_emit_phrase` и partial-пути.
- `communication/chat_app.py`: `_is_accepted_speaker(speaker)` (None/Eddie →
  да, Other → нет) + `_on_phrase(text, speaker=None)` фильтрует чужой голос.
- CDN: eres2net 39.6 МБ (первый вектор может тянуться секунды).
- TDD: `test_speaker_id.py` (8) включая реальный тест на piper-голосах
  (свой 0.735 vs чужой 0.215-0.237); `test_voice_speaker_filter.py` (7).
  Регресс после блока: 67/67.
- Профиль Эдди пока НЕ записан (нужен его голос, ~10-15 с) — до этого
  всё обрабатывается как Эдди (обратная совместимость).

**БЛОК 2 — инициатива и любопытство.**
Корень: `decision_core._local_rules` — если есть активная цель, возвращается
EXECUTE/GENERATE_PLAN/COMPLETE и кандидаты (в т.ч. от любопытства) НИКОГДА не
активируются (L543-579); кд любопытства 1800 с; экран→observe→память без речи.
- `core/curiosity.py`: `min_interval_seconds` 1800 → 600.
- `core/decision_core.py::_local_rules`: кандидат может активироваться при
  свободном слоте активных целей (< MAX_ACTIVE_GOALS=3), при высоком
  frustration (≥0.70) — IDLE вместо активации. Целевая работа не
  вытесняется: слот пополняется. Второй кандидат не активирует поверх
  полных слотов.
- `core/autonomous_runtime.py`: `_maybe_comment_screen()` — спонтанный
  комментарий при НОВОМ описании экрана, кд 3600 с, отсечка коротких и
  пустых описаний; отправка через `eddie_server.send_initiative()`.
- TDD: `test_initiative.py` переписан под реальное поведение (8 тестов):
  активация при свободном слоте, EXECUTE при полных слотах, уступка при
  фрустрации, короткий кд, спонтанные комментарии (новое описание/тот же/
  пустое/кулдаун-окно).

**БЛОК 3 — живой тон (пошаговая стратегия).**
Корень: жёсткий кап длины «1–3 (макс 4)» + «ответ=функция от входа»
(prompt_builder VERBALIZER_BASE:31-39, 55-57), каскад стерилизаторов:
identity_repair.py:187-195 срезал «мне нравится/интересно»,
NEUTRAL-контракт запрещал инициативу (affective_dialogue_policy:569),
repair-промпт запрещал слова «помощь/вопросы» (prompt_builder:211-218).
- `core/prompt_builder.py`: VERBALIZER_BASE — убран жёсткий кап, добавлен
  мандат живой беседы (развивай тему, встречный вопрос, не «функция от
  входа»); VERBALIZER_PERSISTENT_SUFFIX — «вырази живыми словами, можно
  развернуть»; build_repair_prompt — убран запрет «никаких предложений
  помощи/вопросы», добавлен «пара коротких предложений живой реплики».
- `core/identity_repair.py::_repair_unsupported_self_claim`: убрано отрезание
  «мне нравится/мне интересн» (живое отношение остаётся), сохранена защита
  от факт-клеймов о сериалах.
- `identity/affective_dialogue_policy.py` NEUTRAL: вместо «Не добавляй
  искусственную инициативу» — «Не превращай ответ в формальное
  обслуживание», добавлен пункт ведения живого разговора.
- TDD: `test_tone.py` (9 тестов) RED→GREEN.
- Верификация: полный регресс **85/85** (было 53 на старте сессии),
  UTF-8 без BOM (8 файлов), байт-проверка чистая, «?» легитимны
  (SQL-плейсхолдер, вопросительный знак в реплике).

### [АКТУАЛЬНО] Устранено самозакрепление «я голосовой ассистент» (лина LLM-пути)
Эдди: «он снова обозвал себя простым ассистентом, созданным чтобы отвечать
на вопросы... он теперь уверен, что всегда был просто голосовым ассистентом».
Главный принцип проекта: «модель только формулирует ответ EddieAI, а не
отвечает за него». Диагностика (фаза 1 — корень, без правок) выявила цепочку:

1. **Голосовой промпт не сообщал модели о её способностях.**
   Не-голосовые промпты (`prompts.py:215-223,498-508`) содержат блок
   «ТВОИ ОРГАНЫ ЧУВСТВ И ДОСТУП К ПК» (камера/экран/микрофон/ПК),
   а голосовой путь `respond_call_fast` → `build_verbalizer_system_prompt`
   (всего ~2.6 КБ) его НЕ включал. Когда Эдди спросил «у тебя есть функция
   смотреть на экран?», модель честно ответила «я только слышу голос, не
   вижу экран» — она просто не знала о своём зрении.
2. **Самозакрепление через историю.** Каждый такой ответ писался в
   `recent_dialogue` → попадал в контекст следующего запроса → модель
   соглашалась: «я всегда был голосовым ассистентом, это моя сущность»
   (events 2635-2651). Убеждение «застыло» в памяти и кормило каждую новую
   сессию.
3. **Скорость усугубляла** (glm 30-51 с на ответ, zen мёртв 401) — Эдди не
   дожидался и уходил в повторную спираль, как в диалоге 2580-2657.

Изменения (TDD):
- `core/prompt_builder.py`: новая `build_sense_capabilities_block()` —
   единый блок способностей («Ты ЭТО можешь»): камера, микрофон, экран,
   ПК (файлы/терминал/программы/мышь/клавиатура). Без слов «ассистент»/
   «помощник». Требование «не отрицай, что можешь».
- `core/agent.py::respond_call_fast`: голосовой системный промпт теперь =
   `build_verbalizer_system_prompt` + `build_sense_capabilities_block()` +
   инструкция живого разговора.
- `core/agent.py::_sensory_intent_snapshot`: расширены маркеры — добавлены
   «видеть камерой», «смотреть на экран», «функция смотреть», «что ты
   умеешь», «мышк», «клик», «клавиатур» и т.п.; новый канал
   `want_tools` (если спросили про способности без камеры/экрана —
   возвращает описание доступа к ПК).
- Чистка заражённых событий памяти (с бэкапом
   `data/memory_backup_pre_assistant_clean_2026-09-05.db`): удалены 17
   ответов EddieAI, где он называл себя голосовым ассистентом/помощником
   или отрицал зрение (id 2581,2583,2585,2587,2589,2591,2593,2595,2597,
   2629,2635,2637,2639,2641,2645,2649,2651). Корректирующие реплики Эдди
   («ты не голосовой ассистент») СОХРАНЕНЫ — это правильное знание.

Верификация: TDD `tests/test_sense_capabilities.py` 5/5 (RED→GREEN), регресс
53/53 (было 48, +5 новых), UTF-8 без BOM (4 файла), байт-проверка чистая.
Рантайм перезапущен (PID 31380) на новом коде. Живая проба: вопрос
«у тебя есть функция видеть экран или смотреть камерой?» → ответ
«Да, я могу смотреть на экран или через камеру. Что ты хочешь, чтобы я
посмотрел?» (вместо «я голосовой ассистент, не вижу экран»).

### [АКТУАЛЬНО] Свободный, здоровый ритм сна + пробуждение голосом
Эдди: «он спит каждые 2-3 часа постоянно... можно днем спать, если хочет.
просто без такого принудительного засыпания. и должна быть возможность
его разбудить». Корень: `core/life_cycle.py` считал сон от 1.0 до 0.25
со скоростью 0.35/час → 2ч08м сна; лёг с остатком 0.25 → до порога 0.80
набегало ~5-6 часов днём → частые дремы. Это физиология, а не поведение.

Изменения:
- `core/life_cycle.py`: balance под «человеческий» ритм —
  FATIGUE_GAIN_PER_HOUR_AWAKE 0.10 → 0.05 (день копит медленно),
  FATIGUE_LOSS_PER_HOUR_SLEEP 0.35 → 0.10 (ночной сон 7-9 часов),
  WAKE_THRESHOLD 0.25 → 0.15 (просыпается отдохнувшим, без недосыпа).
  Ночь (23:00-07:00, ×1.6) — мягкая подсказка, не принуждение.
  Дневной сон остаётся свободой: если он сам дошёл до порога — спит.
- `identity/sense_listener.py::_finish_phrase`: убран молчаливый игнор фраз
  во сне (`if self._is_asleep(): return`); теперь любая осмысленная фраза
  (>2 слов) будит EddieAI (`_wake_up()` → `life_cycle.force_wake()`) и
  обрабатывается как обращение. Разбудить можно голосом напрямую.
- TDD: `test_life_cycle.py` — полноценный ночной сон 7-9 часов (не 2ч08м),
  дневное бодрствование без принудительных дремот, вечерний естественный
  отбой (корректно с учётом MAX_DT_HOURS=6); `test_sense_listener.py` —
  новый `test_finish_phrase_wakes_when_asleep`: спящий агент будится и
  отвечает. ALL PASS (1 failed → 1 passed при TDD). Байт-проверка UTF-8 без BOM чистая.
- Текущее состояние: после рестарта рантайма EddieAI спит (заснул в 14:02
  до правок, fatigue 0.85) — проспит ~7 ч и проснётся на 0.15, либо будет
  разбужен голосом. Проверено: glm z.ai отвечает по-русски после рестарта
  (HTTP 200) — голосовой канал жив без денег на Zen.

### [АКТУАЛЬНО] Осведомлённость личности о своих возможностях + волевое PERCEIVE
Эдди: «он не умеет пользоваться всеми этими функциями... он опять выдаёт
системный промпт». Диагноз: камера/микрофон/экран/ПК подключены как внешние
автоматические датчики, но ЛИЧНОСТЬ не знала о них (промпты перечисляли только
memory/research/planning/self-model/reflection/contact) и не могла ими управлять
по своей воле. Корень части «выдаёт системный промпт» — отсутствие денег
на облако (вопрос решается отдельно).

Изменения:
- `core/prompts.py`: блок «ТВОИ ОРГАНЫ ЧУВСТВ И ДОСТУП К ПК» в system- и
  quick-промптах — камера/микрофон/экран/терминал/файлы/установка ПО,
  с напоминанием «не выдумывай того, чего не видел» (анти-галлюцинация).
- `identity/action_planner.py`: новое действие PERCEIVE (камера/экран),
  распознавание повелительных форм «посмотри камерой/на экран», «что вокруг»,
  «кто рядом», «скрин/скриншот» и т.п. Остальные правила дополнены
  повелительными формами («проанализируй», «изучи», «подумай»).
- `identity/action_executor.py`: PERCEIVE в VALID_ACTIONS.
- `identity/action_router.py`: PERCEIVE → инструмент `perceive`.
- `identity/tool_policy.py`: PERCEIVE разрешён по умолчанию.
- `identity/tool_runner.py`: `_execute_perceive` (webcam/screen через
  ScreenPerceiver + vision-оркестратор); ToolRunner принимает
  `model_orchestrator` для vision.
- `core/autonomy_runtime_factory.py`: регистрация инструмента `perceive`
  (описание попадает в SelfModel/промпты) + передача оркестратора в ToolRunner.
- `core/agent.py::_sensory_intent_snapshot`: в голосовом звонке, если Эдди
  просит посмотреть камерой/экран — реальный захват + описание втыкается
  в контекст ответа (модель отвечает на основе реальных наблюдений).
- Найден и подтверждён попутный баг (существовал раньше): `_build()` в
  ActionPlanner не имел ветки PERCEIVE → любое нестандартное действие
  молча превращалось в THINK (совпадение ключей не доходило до исполнения).
- TDD `test_perceive_awareness.py` (11: распознавание камеры/экрана/исследования,
  dry-run роут/политика, сенсорный снапшот камеры/экрана, упоминание чувств
  в system/quick) GREEN. Регресс 71 PASS + 54 PASS. Компиляция чистая,
  байт-проверка не показала повреждений кодировок.
...
Учитывая правила: не выходить из роли...»). Корень: respond_call_fast звал
`model_orchestrator.execute(task="conversation")`, а execute БЕЗ metadata["fast"]
форсит `task="deep"` → zen-deepseek-pro (рассуждает/пересказывает, не исполняет
роль). Плюс голосовой контур отвечал даже во сне.
- `core/agent.py::respond_call_fast`: прямой вызов `_cloud_chat(task="conversation")`
  (zen-deepseek-flash, разговорный) с фолбэком на execute→fallback при пустом.
  Живой тест: «Привет, Эдди! Рад тебя слышать.» за 3.16с (было эхо промпта).
- `identity/sense_listener.py::_finish_phrase`: `_is_asleep()` — во сне фраза
  игнорируется (микрофон слушает, но EddieAI молчит, пока спит).
- TDD: 19 тестов PASS. Регресс: 19 PASS. Байт-проверка чистая.

### [АКТУАЛЬНО] Голосовой основной канал + инициатива (мессенджер — редкий)
Решение Эдди: мессенджер — пережиток, основной канал общения = голос
(реального времени). EddieAI слышал, но был «немым» — ответ шёл текстовым
баллоном, озвучка была привязана к voice_mode/звонку.
- `identity/sense_listener.py`: ответ на распознанную речь теперь
  ОЗВУЧИВАЕТСЯ голосом (Piper TTS локально + _boyify подростковый голос,
  без тяжёлого Whisper; VoiceIO.__new__ без __init__ — только синтез),
  и дублируется в мессенджер как agent_message (редкий канал).
- Добавлена ИНИЦИАТИВА: фоновый поток заговаривает сам, если тишина
  дольше initiative_interval (по умолчанию 1800с), не спит и есть
  сигнал от suggest_activity (аффект/интересы) — «Эдди, хочешь ...?».
- TDD test_sense_listener (6: ответ голосом+мессенджер, cam запись/пропуск,
  старт 3 нитей, инициатива при безделье) GREEN. Живой синтез Piper
  проверен (PCM генерируется). Байт-проверка чистая.
...
Полный отчёт `docs_engineer\AUDIT\2026-08-30-audit-all-floors.md`, фрагменты в
workspace SDD. Итог: 6 этажей живы (1,2,3,6,10,12,14-роутинг), 3 частично
(4,5,7), 1 каркас (11), зачатки (8-частично,13,15,14-память). Топ-проблемы
внесены в PROJECT_STATE реестр (п.13–19): мёртвые хуки автономного обучения
(conolidate/reflection signals), сны без событий, зависимость любопытства от
DecisionCore, нет CPU/топ-процессов (этаж 8), двойная STT (этаж 10), нет
обзора структуры проекта (этаж 13), мусор *.backup/mёртвый user.py/таблица
evidence, два дневника. Кодировки всех новых файлов — UTF-8 без BOM.
...
### [АКТУАЛЬНО] 06.09 день: облака Ollama — первые в очереди (решение Эдди)
Эдди: «он должен в приоритете использовать облака от оламы, 2 облачных —
первые в очереди». Контекст жалобы: «чета он все еще не отвечает» — голосом
он ответил дважды в 12:50 и замолчал; glm z.ai после ночной нагрузки
отвечал за ~25 с (рейт-лимит), отказы голоса глотались молча
(`_handle_spoken` без логов), zen-провайдеры (401, ключ истёк) стояли
ПЕРВЫМИ в цепочке.
Правки:
- `core/model_orchestrator.py` — CLOUD_PROVIDERS: первыми добавлены
  ollama-cloud через локального демона (`http://127.0.0.1:11434/v1`,
  модели `gpt-oss:120b-cloud` + `gemma4:31b-cloud`, роли
  conversation/fallback/plan/reflection/affective; gpt-oss —
  `extra_payload {"reasoning_effort": "low"}`, иначе reasoning съедает
  токены и content пустой). Замер: 0.5–1.5 с против 25 с у glm.
  Ключ-файл `~/.eddieai_secrets/ollama.key` = "no-auth" (эндпоинт без
  auth). Тест tests/test_cloud_priority.py 3/3.
- `identity/sense_listener.py` — молчаливые отказы голоса логируются в
  logs/eddie_forever.log (respond_call_fast failed / voice empty answer /
  handle_spoken failed).
- `watchdog_runtime.py` — ollama-демон больше не убивается RAM-guard'ом
  (он теперь реле основного мозга); добавлено самолечение: демона нет на
  11434 → старт `ollama serve`. Убивается только llama-server (локальная
  модель = пожиратель RAM).
- Демон ollama serve поднят (0.32.14, cloud-модели на месте, вход по
  ollama signin — id_ed25519).
Приёмка: живой ответ в чате за 16.8 с («Да, слышу.»), из них LLM ~1-2 с,
остальное — слот цикла 15 с. final_regression 7/7 + 13 pytest PASS.
ИСКЛЮЧЕНИЕ из правила «0 процессов ollama»: демон ollama теперь часть
постоянной инфраструктуры (реле облачного мозга); llama-server
(локальная модель) по-прежнему должен быть 0.

### [АКТУАЛЬНО] 06.09 день (2): EddieAI впервые видит — глаза через ollama-gemma
...
zen-kimi (ключ истёк, 401) → `ScreenPerceiver._vision_describe` и
`webcam_describe` ВСЕГДА возвращали пустоту: 0 снимков экрана за всю
историю (source='screen' в БД отсутствует), вебка молчала. Промпт-блок
способностей обещал зрение, которого физически не было.
Правка: `core/model_orchestrator.py` — роль "vision" добавлена
ollama-gemma (gemma4:31b-cloud, мультимодальная). Проверено живьём:
тестовая картинка описана верно за 1.2 с; вебка-проба описала реальную
комнату («человек в светлой одежде… кровать, занавеска»). После рестарта
первое визуальное событие в прод-памяти: «Камера: В кадре видна часть
кровати…» (SHARED_EXPERIENCE, source=webcam, 13:27). Экранные
WORLD_SNAPSHOT пойдут с тиком ScreenPerceiver (120 с).
Цепочка «функций» проверена: perceive-инструмент зарегистрирован
(tool_runner:426), `_sensory_intent_snapshot` смотрит камерой/экраном
живьём при просьбе («посмотри камерой»), spontaneous-комментарии экрана
(`_maybe_comment_screen`) теперь получат описания. test_perceive_awareness
11/11, test_cloud_priority 3/3, final_regression PASS.

### [АКТУАЛЬНО] 06.09 день (3): событийное восприятие — окна, значимая смена, слухи ПК
Эдди одобрил план («первый шаг к тому, чтобы он мог комментировать сам,
говорить сам, делать всё сам»). Этапы 1–2 реализованы (TDD):
- `identity/screen_perceiver.py` — фикс захвата экрана НАВСЕГДА сломанного:
  `with sct.grab()` в установленном mss больше не context-manager →
  TypeError → except → None → экран не видели ни разу. Стало `img =
  sct.grab()`. Плюс `changed_significantly()` (поканальная разница RGB на
  160×90, порог 2% заметных пикселей) — мигание курсора не тратит лимит;
  `active_window_title()` (ctypes) — заголовок активного окна в промпт
  vision и в событие «Экран [окно: …]».
- `identity/pc_audio_listener.py` (новый) — слухи ПК: системный звук через
  Voicemeeter-кабель (выбор устройства по кандидату «voicemeeter out /
  stereo mix»), Vosk 16k (декимация при 48k), фильтры ≥4 слов и кд 45 с,
  запись «Слышно с компьютера: …» (WORLD_SNAPSHOT, source=pc_audio) +
  уровень RMS раз в минуту в лог. Подключён в run_forever (старт/стоп).
- Приёмка (14:03): экран описывается с заголовком окна («(555) CEHR
  ВПИТЫВАЕТ БАЗУ… НАРУТО» — Eddie смотрит видео), вебка видит Эдди
  («молодой человек в наушниках»), pc-audio слушает A4 (RMS-наблюдение в
  логе; маршрут системного звука проверить при реальном воспроизведении).
  Тесты: screen_change 5, pc_audio 5, регресс final 7/7.
- Дальше по плану (Этап 3): режим «смотрим вместе» (смена сцены → кадр +
  диалоги фильма), Этап 4: UIA-чтение текста активного окна.

### [АКТУАЛЬНО] 06.09 день (4): этапы 3+4 — «смотрим вместе» и текст окна, вход по своей воле
Эдди: «делай сейчас, конечно. и 3 и 4. и чтобы он сам мог войти в эти
режимы, конечно».
- `identity/watch_mode.py` (новый) — режим «смотрим вместе»: автономный
  вход (маркер видео в заголовке активного окна, устойчивый ≥60 с),
  выход по тишине (нет новых кадров И нет звука ≥300 с), комментарии раз
  в 600 с; принудительный вход/выход — по команде совместной активности
  «фильм» (eddie_server._apply_shared_activity_command хук).
- `identity/window_text.py` (новый) — текст сфокусированного элемента
  через UI Automation (comtypes): фрагмент идёт в промпт описания экрана.
- `core/autonomous_runtime.py` — `_handle_watch_mode` (каденция экрана
  120с→40с в режиме, note_motion/audio, вход/выход, комментарий) +
  `_watch_comment` (cloud_chat_stream → send_initiative, живая реакция
  голосом/чатом). Экранные события переключения окон («Эдди
  переключился на окно: …») в ScreenPerceiver.
- Слухи ПК: last_audio_ts (не-тишина держит режим).
- Проводка: фабрика (runtime._watch_mode, agent.watch_mode),
  run_forever (agent.pc_audio). Тесты test_watch_mode 7/7; итог регресс
  final 7/7 + 41 pytest.
- ПРИЁМКА (14:32): «Включаюсь: смотрим вместе.» — САМОстоятельный вход
  (YouTube на экране); первая реакция ожидается к ~14:42.
- Правка по приёмке: num_predict реакции 120→300 (reasoning gpt-oss
  съедал 120 → пустой контент). Проба: «Ничего себе, как же это
  круто!» — реакция живая. Повторный вход в режим — при следующем видео
  на экране (сейчас Эдди в ZCode — вход по маркеру видео корректно не
  срабатывает).
- Слухи ПК переведены на WASAPI-loopback основного выхода (решение
  подтверждено Эдди: Voicemeeter — только раздача наушников, не основной
  канал). `pick_loopback_device` (по имени дефолтного выхода +
  префикс-fallback), PyAudioWPatch (pip), стерео→моно + декимация 48→16к;
  sounddevice-кабели остаются запасным путём. Тесты 8/8. Приёмка: тон в
  дефолтный выход → rms=0.2769 в логе (был 0.0000).
### [АКТУАЛЬНО] 06.09 вечер (2): чистое зрение + пауза видео при репликах
Эдди: «он говорит, что ему какой-то глич мешает смотреть»; «должен ставить
на паузу, чтобы сказать, и снимать её, если закончил мысль и не ждёт
ответа. не как закон, а удобство».
- КОРЕНЬ ГЛИТЧА: mss raw — BGRA (4 байта/пиксель), читалось как RGB →
  диагональный сдвиг строк → «помехи и глитч-эффекты» в его описаниях.
  Фикс: ScreenShot.rgb (правильная конверсия). Живая проверка: описание
  экрана точное, читает текст UI («ZCode… Ask for follow-up changes»).
- Паузы: watch_mode.pause_decision (аудио играет + реплика ≥40 симв);
  media-клавиша Play/Pause (VK 0xB3, keybd_event) перед репликой;
  resume_auto = оценка длительности речи (len/13 + 3с); вопрос Эдди →
  resume_hold 180с (страховка от зависшей паузы). Тесты 10/10.

### [АКТУАЛЬНО] 06.09 вечер (3): контур «слово → дело» + чистое зрение + паузы
Эдди: «все 3 нужно делать» (варианты решения «роль модели»).
- identity/speech_promises.py (новый): обещания первого лица → карта
  глаголов на способности; правило 1 — обещание с инструментом становится
  целью ядра (speech_promise, кд 300с); правило 2 — обещание без
  способности вырезается с объектом; правило 3 — «уже сделал» сверяется
  с фактами инструментов за 15 мин, без факта — вырезается.
- Хуки: eddie_server.respond_and_deliver + sense_listener._handle_spoken
  (чат и голос). Тесты 8/8, регресс final 7/7.
- Глитч зрения: mss raw BGRA читался как RGB → диагональный сдвиг; фикс
  ScreenShot.rgb. Паузы видео: pause_decision + media-клавиша + resume_auto
  (оценка речи) / resume_hold (вопрос → 180с). Тесты 10/10.
- Self-model: ограничения перцепции живьём (пересборка после полной
  проводки чувств) — «нет слуха» больше не ложно.
### [АКТУАЛЬНО] 06.09 вечер (4): две камеры с ролями + cloud-only
Эдди: «зачем грузит оламу на локалке?», «подключил более нормальную
камеру — вид на всю комнату; вебка ноута чтобы мне в ебало посмотреть».
- Диагноз: llama-server 1.3 ГБ поднимал локальный фолбэк (phi4-mini при
  сбое облака и RAM ≥2.6 ГБ). УТОЧНЕНИЕ Эдди: локальные — последний
  рубеж («если прям вообще никак»), не запрет: облака мертвы → RAM-guard
  → ответ → выгрузка сразу (все keep_alive 3m → 0, модель не висит в
  RAM). ollama-демон остаётся реле ollama-cloud (45 МБ).
- Камеры: room=2 (новая USB — обзор комнаты), face=0 (ноутбучная — лицо);
  Snap Camera исключена. Приёмка: «Камера (комната): молодой человек
  смотрит прямо в камеру» (18:08).
### [АКТУАЛЬНО] 06.09 вечер (5): рубеж D этап 2 — тело, микромысли, рассказчик; голос с внутренней жизнью
Эдди: «мне обидно видеть, что он просто не живой»; «почему в чате живее,
 чем говорит?».
- Тело (identity/body.py): соц. нужда (3ч до «скучаю по Эдди») и новизна
  (2ч однообразия) — растут, удовлетворяются реальным контактом/событиями,
  вкладываются в порог речи. Тесты 7/7.
- Микромысли (ThinkRhythm ~3 мин, без LLM): всплывают воспоминания за
  48ч, счётчик тишины, дело, настроение — поток живёт и в тишине.
- Рассказчик (раз в час): нарратив в память (REFLECTION/narrator) и поток.
- Голос: respond_call_fast получил настроение+поток+экран (был слеп —
  только 6 строк диалога; потому письменные инициативы были живее
  голоса), num_predict 300.
- Плюс: контур «слово → дело» (обещания→цели, пустые—вымываются,
  «уже сделал»—по факту) — решение Эдди «все 3» (e92d1e2); чистый захват
  экрана (BGRA), паузы видео; self-model с живыми ограничениями перцепции.
### [АКТУАЛЬНО] 06.09 ночь (2): «пошаговая стратегия» → разговор = обмен прожитым
Эдди: «решим, почему наши разговоры это пошаговая стратегия, а не общение?». Диагноз-связка: правило 4 («не выдумывай сверх входных») + голосовой контекст 6 строк без внутренней жизни = ему нечего принести, кроме уточняющего вопроса; рамка «ты — речевой аппарат» = рот вместо собеседника; «Эдди,» в каждой фразе — выученный маркер без фильтра.
Фиксы: мандат КАК ВЕСТИ РАЗГОВОР в вербализатор (неси своё — мнение/воспоминание/наблюдение; веди нить темы; обращение изредка; вопрос — макс 1); правило 4 переписано (внутренняя жизнь и память — его правда, говорить смело); голос: окно диалога 6→20, НЕДАВНЕЕ МЕЖДУ ВАМИ (интересы+предпочтения) в промпт, срез дежурного «Эдди,» в общем контуре речи. Тесты 38 целевых + регресс 7/7. Коммит 7ba811c.
### [АКТУАЛЬНО] 06.09 ночь (3): веб-мессенджер «Tabula» в стиле Рима
Эдди: «мессенджер неудобный, зависает; должен быть наш личный телеграм, локальный».
Реализовано communication/web_messenger.py: http://127.0.0.1:7779 (только localhost),
единая история из chat_history (включая [голос]), WebSocket-доставка (сообщения/
инициативы/печатает), автопереподключение; дизайн позднереспубликанского Рима:
papyrus-свиток, восковые таблички, бронза, EDDIE·AI, S.P.Q.E. Сообщения Эдди идут
тем же конвейером handle_user_message. tkinter-чат пока жив как TTS/трей-двигатель.
Тесты 2/2; HTTP 200 за 6 мс; RAM ~60 МБ. Коммит.
### [АКТУАЛЬНО] 06.09 ночь (4): три разрыва живой связи — найдены и сшиты
Эдди: «не отвечает в мессенджере, молчит голосом». Три разрыва: (1) TCP-мост
падал на каждой инициативе — payload→msg (моя ночная правка диспетчера);
(2) воля ставила чтение чата ПОСЛЕ целей — сообщения ждали неопределённо;
(3) голос gpt-oss отдавал пустоту (thinking съедал 300 токенов) → 400.
Живое подтверждение: «что смотрим?» прочитано и отвечено экраном (599-601),
энергосбережение сработало живьём на 15% батареи с объявлением Эдди.
Тесты 38+6 целевых, регресс 7/7. Коммиты 46124d7/7f58abb/784da98.
