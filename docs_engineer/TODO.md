...
### [06.09 вечер] РУБЕЖ D «Внутренний поток» — ядро запущено
Эдди: «почему он всё ещё не живой» → диагноз: органы есть, расписания
есть, СУБЪЕКТА нет (сознание = вызовы по расписанию). Реализовано:
- [x] InnerStream (поток сознания, urgency, инициатива из порога) +
      Mood (настрой-фон, дрейф к аффекту, персистентность) — 10/10 тестов.
- [x] Спека SPECS/2026-09-06-rubezh-d-inner-stream.md + наблюдение:
      настроение в self_state обновляется; в тишине молчит (корректно).
- [ ] Следующее: тело (гомеостаз), рассказчик (нарратив), внимание с
      приоритетами, витубер-слой (latency/аффект в голосе/аватар).
- [ ] Наблюдать: первые спонтанные реплики из потока при активном экране.
...
- [x] Хвосты закрыты (отмашка Эдди 06.09 ~17:00): КОММИТЫ 514c064/2cf799b/c60eb03
      (36+ файлов, дерево чистое); семсудья — честное исследование
      (NO_ACCEPTED_SOURCES → OK + honest_note, external_records пусты);
      калибровка доменов SourceEvaluator (habr/stackoverflow/python.org/
      arxiv и др.); мусор temp/, tmp_*.py удалён; .gitignore без BOM.
